@auth
    ...
    @if($popupNotifications->isNotEmpty())
        <div x-data="{
            ...
            {{-- Backdrop --}}
            <div class="absolute inset-0 bg-black/50" @click="if(currentPopup) closePopup(currentPopup)"></div>
            ...
            @foreach($popupNotifications as $notification)
                ...
                <template x-if="popups.find(p => p.id === {{ $notification->id }})?.visible">
                    <div x-transition:enter="transition ease-out duration-200"
                         ...
                         class="relative w-full max-w-md bg-surface border border-oat rounded-card shadow-lg overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex items-start gap-3 mb-4">
                                ...
                                <button type="button"
                                        ...
                                        class="flex-shrink-0 p-1 text-muted hover:text-off-black transition-colors rounded-btn hover:bg-canvas">
                                    <i data-lucide="x" class="w-4 h-4"></i>
                                </button>
                            </div>

                            {{-- Message body --}}
                            <div class="text-sm text-off-black leading-relaxed">
                                {{ $notification->message }}
                            </div>
                        </div>

                        {{-- Footer actions --}}
                        <div class="px-6 py-4 bg-canvas border-t border-oat flex items-center justify-end gap-3">
                            <button type="button"
                                    :disabled="popups.find(p => p.id === {{ $notification->id }})?.dismissing"
                                    @click="dismissPopup(popups.find(p => p.id === {{ $notification->id }}))"
                                    class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-off-black bg-transparent border border-off-black rounded-btn hover:scale-105 active:scale-95 transition-transform">
                                <i data-lucide="bell-off" class="w-3.5 h-3.5"></i>
                                <span x-show="!popups.find(p => p.id === {{ $notification->id }})?.dismissing">Don't remind me</span>
                            ...
                            <button type="button"
                                    @click="closePopup(popups.find(p => p.id === {{ $notification->id }}))"
                                    class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-off-black rounded-btn hover:scale-105 active:scale-95 transition-transform">
                                Tutup
                            </button>
                        </div>
                    </div>
